...
pub(crate) fn create_irradiance_map(
    ...
    // create cubemap
    let irradiance_map =
        Texture::create_renderable_cubemap(context, size, 1, vk::Format::R32G32B32A32_SFLOAT,std::ffi::CString::new("Irradiance Texture").unwrap());
    ...
    log::info!("irradiance map生成结束，耗时{}ms", time);

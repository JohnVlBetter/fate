...
pub struct Nodes {
    nodes: Vec<Node>,
    indices: Vec<(usize, Option<usize>)>,
}
...
impl Nodes {
    ...
    fn new(nodes: Vec<Node>, roots_indices: Vec<usize>) -> Self {
        let indices = build_graph_run_indices(&roots_indices, &nodes);
        Self { nodes, indices }
    }
}
...
impl Nodes {
    pub fn transform(&mut self, global_transform: Option<Matrix4<f32>>) {
        for (index, parent_index) in &self.indices {
...
fn build_graph_run_indices(roots_indices: &[usize], nodes: &[Node]) -> Vec<(usize, Option<usize>)> {
    ...
    for root_index in roots_indices {
        dfs(nodes, *root_index, None, &mut indices);
    }
    indices
}
...
fn dfs(
    ...
    for child_index in &nodes[node_index].children_indices {
        dfs(nodes, *child_index, Some(node_index), indices);
    }
}
